...
public class UserService(IUserRepository userRepository, UserMapper mapper)
{
    ...
    /// <summary>
    /// Деактивирует пользователя. Счета с нулевым балансом закрываются,
    /// счета с остатком остаются открытыми, но все карты блокируются.
    /// </summary>
    public async Task<DeactivateResponse> Deactivate(int id)
    {
        var user = await userRepository.GetByIdWithAccountsAsync(id)
            ?? throw new AppException(ErrorCodes.UserNotFound, HttpStatusCode.NotFound);
